...
$SQLString = SqlQueryGenerator::generateFrQuery($pled, $direcziun, $modus);
...
<div class="cuntegn">
    <div class="head">
        <div class="header">
            <a class="title accent-a" href=".">monPledari</a>
            <div class="slogan">
                <div>Pledari rumantsch-franzos</div>
                <div>Dictionnaire français-romanche</div>
            </div>
        </div>

        <form name="tschertga" method="post" action="index.php" class="search">
            <label>
                Direcziun per la tschertga / Direction de la recherche<br>
                <select name="direcziun" onChange="go()">
                    <option value="0" <? if($direcziun == 0){ echo "selected"; } ?>>Français -> Romancio (RG)</option>
                    <option value="1" <? if($direcziun == 1){ echo "selected"; } ?>>Rumantsch (RG) -> Français</option>
                    <option value="2" <? if($direcziun == 2){ echo "selected"; } ?>>Sursilvan -> Français</option>
                    <option value="3" <? if($direcziun == 3){ echo "selected"; } ?>>Sutsilvan -> Français</option>
                    <option value="4" <? if($direcziun == 4){ echo "selected"; } ?>>Surmiran -> Français</option>
                    <option value="5" <? if($direcziun == 5){ echo "selected"; } ?>>Puter -> Français</option>
                    <option value="6" <? if($direcziun == 6){ echo "selected"; } ?>>Vallader -> Français</option>
                    <option value="7" <? if($direcziun == 7){ echo "selected"; } ?>>Rumantsch (total) -> Français</option>
                    <option value="7" <? if($direcziun == 8){ echo "selected"; } ?>>Intercurir remartgas</option>
                    <option value="9" <? if($direcziun == 9){ echo "selected"; } ?>>Rechercher commentaires</option>
                </select>
            </label>

            <fieldset>
                <legend>Modus per la retschertga / Mode de recherche</legend>
...
?>
    ...
    <div class="results">
        <div class="results-column" id="results-left">
            <div class="result">
                <div class="result-label">Franzos:<br>Français::</div>
        ...
        <div class="results-column" id="results-right">
            <div class="result" id="result-comments">
                <div class="result-label">Remarques:</div>
                <div class="result-content">
