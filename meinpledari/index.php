...
<div class="cuntegn">
    <div class="head">
        <div class="header">
            <a class="title accent-a" href=".">meinPledari</a>
            <div class="slogan">
                <div>Pledari rumantsch-tudestg</div>
                <div>Deutsch-Romanisches Wörterbuch</div>
            </div>
        </div>

        <form name="tschertga" method="post" action="index.php" class="search">
            <label>
                Direcziun per la tschertga / Suchrichtung
                <select name="direcziun" onChange="go()">
                    <option value="0" <? if($direcziun == 0){ echo "selected"; } ?>>Deutsch -> Romansh (RG)</option>
                    <option value="1" <? if($direcziun == 1){ echo "selected"; } ?>>Rumantsch (RG) -> Deutsch</option>
                    <option value="2" <? if($direcziun == 2){ echo "selected"; } ?>>Sursilvan -> Deutsch</option>
                    <option value="3" <? if($direcziun == 3){ echo "selected"; } ?>>Sutsilvan -> Deutsch</option>
                    <option value="4" <? if($direcziun == 4){ echo "selected"; } ?>>Surmiran -> Deutsch</option>
                    <option value="5" <? if($direcziun == 5){ echo "selected"; } ?>>Puter -> Deutsch</option>
                    <option value="6" <? if($direcziun == 6){ echo "selected"; } ?>>Vallader -> Deutsch</option>
                    <option value="7" <? if($direcziun == 7){ echo "selected"; } ?>>Rumantsch (total) -> Deutsch</option>
                    <option value="9" <? if($direcziun == 9){ echo "selected"; } ?>>Bemerkungen durchsuchen</option>
                </select>
            </label>

            <fieldset>
                <legend>Modus per la retschertga / Suchmodus</legend>
                <label>
                    <input type="radio" name="modus" value="entschatta" class="buccolur" <? if($modus == "entschatta"){ echo "checked"; } ?> checked>
                    normal / beginnt mit<br>
                </label>
                <label>
            ...
            <label>
                Term da tschertgar / Suchbegriff<br>
                <input type="text" name="pled" value="<? if(@$pled_ei_datum != 1){echo $pled; } ?>">
            </label>

            <input type="submit" name="tschertgar" value="Tschertgar / suchen">
        </form>
    </div>
...
?>
    ...
    <div class="results">
        <div class="results-column" id="results-left">
            <div class="result">
                <div class="result-label">Tudestg:<br>Deutsch:</div>
